...
@Composable
fun MostViewedPhotosScreen(state: State<AppState>, navController: NavHostController) {
    Box(modifier = Modifier.padding(top = 100.dp)) {
        ...
        LazyColumn(modifier = Modifier.fillMaxWidth()) {
            items(state.value.mostViewedPhotos!!.size) { index ->
                val photo = state.value.mostViewedPhotos!![index]
                Image(
